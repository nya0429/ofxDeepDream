...
class Network(torch.nn.Module):
    ...
    def forward(self, tenFirst, tenSecond):
        ...
        for intLevel,basic in enumerate(self.netBasic):
            if intLevel < len(tenFirst):

            # for intLevel in range(len(tenFirst)):
                tenUpsampled = (
                    torch.nn.functional.interpolate(
                        tenFlow, scale_factor=2.0, mode="bilinear", align_corners=True
                    )
                    * 2.0
                )
                ...
                if tenUpsampled.shape[2] != tenFirst[intLevel].shape[2]:
                    tenUpsampled = torch.nn.functional.pad(
                        tenUpsampled, pad=[0, 0, 0, 1], mode="replicate"
                    )
                if tenUpsampled.shape[3] != tenFirst[intLevel].shape[3]:
                    tenUpsampled = torch.nn.functional.pad(
                        tenUpsampled, pad=[0, 1, 0, 0], mode="replicate"
                    )
